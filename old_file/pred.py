...
from step2_logics import logic_1, logic_2
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
sys.path.append(os.path.join(os.path.dirname(__file__), '../..'))


from logics import logic3, logic4, logic5, logic6, logic7, logic8, logic9
...
def predict(use_logic, file_name, table_len, table_list):
    ...
    #     target_table1 = logic1.logic1(file_name, table_len, table_list)
    #     target_tables.append(target_table1)

    print("logic2 start...")
    if "logic2" in use_logic:
        target_table2 = logic_2.logic2(file_name, table_len, table_list)
...
def predict_check_result(predict_shops, logics):
    check_results = []
    pred_lists = []
    ...
    for predict_shop in predict_shops:
        ...
        target_tables = predict(logics, file_name, table_len, table_list)
...
if __name__ == '__main__':
    ...
    # 適用するロジック（1ヶ月の成績が5割以上 or 上位3つ
    logics=["logic2", "logic3", "logic7"]

    predict_shops = ["gandam_yunicorn_rakuen_nannba"]
    predict_check_result(predict_shops, logics)
